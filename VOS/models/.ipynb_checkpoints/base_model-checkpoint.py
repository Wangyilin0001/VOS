...
class BaseModel():
    ...
    def load_networks(self, which_epoch):
        for name in self.model_names:
            if isinstance(name, str):
                ...
                # hook
                net.model[10].conv_block[1].register_forward_hook(self.get_activation('model10'))

                print('111111111',net.model[10].conv_block[1].register_forward_hook(self.get_activation('model10')))
    ...
    def get_activation(self,name):
        def hook(model, input, output):
            self.input_data[name] = input[
                0].detach()  # input type is tulple, only has one element, which is the tensor
            self.output_data[name] = output.detach()  # output type is tensor
