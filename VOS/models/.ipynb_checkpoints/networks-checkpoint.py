...
def define_G1(input_nc, output_nc, ngf, which_model_netG1, norm='batch', use_dropout=False, init_type='normal',
    ...
    if which_model_netG1 == 'resnet_9blocks':
        ...
        netG1 = MSUNet()
    elif which_model_netG1 == 'MSUNet_me_G1':
        netG1 = MSUNet_me()
    elif which_model_netG1 == 'MSUNet_me_3_G1':
        netG1 = MSUNet_me_3()
...
def define_G2(input_nc, output_nc, ngf, which_model_netG2, norm='batch', use_dropout=False, init_type='normal',
    ...
    if which_model_netG2 == 'resnet_9blocks':
        ...
        netG2 = MSUNet()
    elif which_model_netG2 == 'MSUNet_me_G2':
        netG2 = MSUNet_me()
    elif which_model_netG2 == 'MSUNet_me_3_G2':
        netG2 = MSUNet_me_3()
...
class TRBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, stride=1, shortcut=None):
        ...
        self.up = nn.Sequential(
            DCnv2(in_ch, out_ch, kernel_size=3, stride=1, padding=1),  # 卷积之后，如果接BN操作，就不要设置偏置，因为不起作用还要占显卡内存。
        )
        ...
        self.down = nn.Sequential(
            DCnv2(in_ch, out_ch, kernel_size=3, stride=1, padding=2, dilation=2),
        )
        ...
        self.dila = DCnv2(out_ch,out_ch,kernel_size=3,stride=1,padding=1)  # 因为之后没有接BN所以需要使用偏执
...
class MSUNet_me_3(nn.Module):
    def __init__(self, in_channels=3, n_classes=3, feature_scale=4, is_deconv=True,
        ...
        self.h04_PT_hd13 = Canchakuai_me(in_ch=filters[4], out_ch=filters[4], BN = True,shortcut=None)
        ...
        self.conv22 = nn.Sequential(nn.Conv2d(filters[2]+self.CatChannels+self.CatChannels,filters[2], 1),
        ...
        self.conv31 = nn.Sequential(nn.Conv2d(filters[1]+self.CatChannels, filters[1], 1),
    ...
    def forward(self, inputs):
        ...
        # 此处可以设计一个14
        X_13 = self.up_concat13(self.h04_PT_hd13(X_04), X_03)  # 第一个为需要上采样的（尺寸变大两倍）   输出：（1，256，w/8，h/8）
        ...
        X_22 = torch.cat((X_22,h00_hd22, h01_hd22), 1)  # 128+64+64 =256
        X_22 = self.conv22(X_22)  # （1，256，w/4，h/4）  ->  （1，128，w/4，h/4）
        # 第三列输出
...
class DCnv2(nn.Module):
    def __init__(self, c1, c2, kernel_size=3, stride=1, padding=1, groups=1, dilation=1,act=True,):
        super(DCnv2, self).__init__()

        self.conv1 = nn.Conv2d(c1, c2, kernel_size=kernel_size, stride=stride, padding=padding, groups=groups,dilation=dilation, bias=False)
        deformable_groups = 1
        offset_channels = 18
        mask_channels = 9
        self.conv2_offset = nn.Conv2d(c2, deformable_groups * offset_channels, kernel_size=kernel_size, stride=stride, padding=padding,dilation=dilation)
        self.conv2_mask = nn.Conv2d(c2, deformable_groups * mask_channels, kernel_size=kernel_size, stride=stride, padding=padding,dilation=dilation)
        # init_mask = torch.Tensor(np.zeros([mask_channels, 3, 3, 3])) + np.array([0.5])
        # self.conv2_mask.weight = torch.nn.Parameter(init_mask)
        self.conv2 = DeformConv2d(c2, c2, kernel_size=kernel_size, stride=stride, padding=padding, bias=True, dilation=dilation)
