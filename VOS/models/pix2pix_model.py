...
class Pix2PixModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['G1_GAN','G2_GAN', 'G1_L1', 'G2_L1','D1_real','D2_real', 'D1_fake','D2_fake', 'G1', 'G2','D1','D2']
    ...
    def backward_G1(self):
        ...
        if self.l1 ==0:   # 此时没有a_I的分割损失
            self.loss_a_II = k*self.criterionL1(self.fake_a_II, self.real_Bab_II)   # a_II的分割损失
            self.loss_a = self.loss_a_II
        else:
            self.loss_a_I = k * self.criterionL1(self.fake_a_I, self.real_Ba_I)  # a_I的分割损失
            self.loss_a_II = k * self.criterionL1(self.fake_a_II, self.real_Bab_II)  # a_II的分割损失
            self.loss_a = (self.loss_a_I + self.loss_a_II) / 2
    ...
    def optimize_parameters(self,l1,l2,line):
        self.forward(l1,l2)   # self.fake_B = self.netG(self.real_A)  得到生成器生成的图像
        # 更新 D1
        if self.use_gan == 1:
